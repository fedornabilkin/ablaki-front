...
const request = usePageRequest(() => lobbyStakes(props.kind, props.scope), [] as HistoryKon[], [
  () => props.kind, () => props.scope, () => props.version, computed(() => store.state.auth.revision),
]);
const unit = computed(() => props.kind === 'saper' ? 'Кг' : 'Cr');
const missingSelected = computed(() => props.modelValue && !request.data.value.some(item => Number(item.kon) === Number(props.modelValue)));
</script>
<template lang="pug">
.stake-filter(role="group" aria-label="Фильтр по ставке" :aria-busy="request.loading.value")
  ...
  n-button(v-for="item in request.data.value" :key="item.kon" size="small" :type="Number(modelValue) === Number(item.kon) ? 'primary' : 'default'" :aria-pressed="Number(modelValue) === Number(item.kon)" :disabled="disabled" @click="emit('update:modelValue', item.kon)") {{ Number(item.kon) }} {{ unit }} · {{ item.count }}
  n-button(v-if="missingSelected" size="small" type="primary" aria-pressed="true" :disabled="disabled" @click="emit('update:modelValue', '')") {{ modelValue }} {{ unit }} ×
