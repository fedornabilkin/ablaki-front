...
const { data, loading } = usePageRequest(() => props.count > 0 ? list(`forum-comment/${commentId.value}/gifts`, 1, { 'per-page': 100 }) : Promise.resolve(emptyPage()), emptyPage(), [commentId, () => props.count]);
...
.gift-users(v-if="count > 0")
  n-skeleton(v-if="loading" width="6rem" height="1rem" aria-label="Загрузка благодарностей")
  template(v-if="!loading")
    template(v-for="(user, index) in visibleUsers" :key="user.id")
      | {{ index ? ', ' : ' ' }}
      router-link(:to="'/wall/' + encodeURIComponent(field(user.username))" :title="(Number(user.amount) || 1) + ' Cr'") @{{ field(user.username) }}
