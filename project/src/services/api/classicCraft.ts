...
export interface CraftState { items: CraftItem[]; recipes: CraftRecipe[]; categories: {id: number; name: string; code: string}[]; stations: {id: number; name: string; item_id: number | null}[]; skills: {category_id: number; experience: number; level: number}[]; inventory: {item_id: number; quantity: number}[]; credit: number; starter_available: boolean; gather_available: boolean; slot_limit: number; slots_used: number }
...
export interface CraftCommand { action: CraftAction; id: number; quantity: number; request_key: string }
...
export function parseCraftState(value: unknown): CraftState {
  ...
  const state: CraftState = {
    ...
    categories: arr(d.categories, v => { const r = object(v); return {id: num(r.id, 1), name: str(r.name), code: str(r.code)}; }),
    ...
    credit: num(d.credit, 0, false), starter_available: bool(d.starter_available), gather_available: bool(d.gather_available), slot_limit: num(d.slot_limit, 1), slots_used: num(d.slots_used),
  };
  for (const rows of [state.items, state.recipes, state.categories, state.stations]) if (new Set(rows.map(r => r.id)).size !== rows.length) throw Error('invalid-response');
