...
export function useClassicCraft(session: Ref<number>, refreshAccount: () => Promise<unknown>, owner?: Ref<number>) {
  ...
  function restore(): CraftCommand | null {
    try {
      ...
      const inventoryAction = value && ['use', 'discard'].includes(value.action);
      const validSlot = value && (value.slot_id === undefined || (inventoryAction && Number.isSafeInteger(value.slot_id) && value.slot_id > 0));
      if (value && validSlot && ['craft','starter','gather','use','discard'].includes(value.action) && Number.isSafeInteger(value.id) && value.id >= 0 && Number.isSafeInteger(value.quantity) && value.quantity >= 1 && value.quantity <= (inventoryAction ? 10000 : 100) && typeof value.request_key === 'string' && /^[A-Za-z0-9_-]{16,80}$/.test(value.request_key)) return value;
  ...
  function command(action: CraftAction, id = 0, quantity = 1, slotId?: number) {
    ...
    return send({action, id, quantity, ...(slotId === undefined ? {} : {slot_id: slotId}), request_key: Array.from(bytes, n => n.toString(16).padStart(2, '0')).join('')});
  }
