...
export function useClassicCraft(session: Ref<number>, refreshAccount: () => Promise<unknown>, owner?: Ref<number>) {
  ...
  function restore(): CraftCommand | null {
    try {
      ...
      const validPrice = value && (value.action === 'buy_slots' ? Number.isSafeInteger(value.unit_price) && value.unit_price >= 0 && value.unit_price <= 30000000 && (value.total_price === undefined || (Number.isSafeInteger(value.total_price) && value.total_price >= 0 && value.total_price <= 465000000)) : value.unit_price === undefined && value.total_price === undefined);
