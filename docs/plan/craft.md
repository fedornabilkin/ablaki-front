...
> Обновление 10.09.2026: ниже сохранён исторический план с предполагаемым API. В backend master `63855e2` появился модуль крафта, но контроллеры REST ещё отсутствуют, а модель данных отличается от мока. Актуальные маршруты, ограничения и изменения фронта описаны в [аудите API](../craft-api-audit.md). Переключение env само по себе не подключает серверный крафт.
