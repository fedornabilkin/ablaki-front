...
> Актуализация 20.09.2026: исторические отметки ниже относятся к демо, не к серверной экономике. Backend-модуль craft уже существует, но требует завершения API и операций. Активный план: [классический крафт](../2026-09-20-classic-crafting.md). user/persone.credit — единственный аккаунт/баланс. Таймер и биржа отложены. Переключение одной env-переменной не является интеграцией.

## Контекст

На бэке крафта нет (модули `back/yii2/common/modules/` — только `exchange`, `forum`, `games`; никаких `item`/`recipe`/`inventory`/`craft` ни в моделях, ни в миграциях, ни в REST). План для бэка описан отдельно в `back/docs/plan/done/craft.md` (нумерация `д.*`).
